...
import {
  ...
  fetchDeclinedReasonList,
  fetchCampaignList,
  fetchVendorList,
  fetchDeclinedData,
} from "~/redux/actions/payees";
import ColumnSettings from "~/modules/ColumnSettings";
import DeclinedData from "~/modules/PayeesData/DeclinedData";
...
class Payees extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      vendorList:[],
      selectedCampaignId: " ",
      selectedCurrencyDeclined: "USD",
    };
  }

  componentDidUpdate(prevProps, prevState) {
      if ((prevState.selectedVendorId !== this.state.selectedVendorId) || (prevState.selectedCampaignId !== this.state.selectedCampaignId)) {
          const {selectedVendorId, selectedCampaignId} = this.state;
          this.getDeclinedData(selectedVendorId, selectedCampaignId); 
  ...
  getColumnListsName = () => {
    this.props.dispatch(fetchColumnsList("Declined")).then((response) => {
      if (!response) {
        this.setState({
          alertType: "error",
          alertMessage: this.props.payees.error,
      ...
      this.setState({
        columns: this?.props?.payees?.columnList.map((item) => ({
          ...item,
          isChecked: item.isCheckDeclined,
          key: item.infokey,
        })),
      });
    });
  ...
  getDeclinedReasonList = () => {
    this.props.dispatch(fetchDeclinedReasonList()).then((response) => {
      if (!response) {
        this.setState({
          alertType: "error",
          alertMessage: this.props.payees.error,
      ...
      this.setState({
        declinedReasonList: this.props.payees.declinedList,
      });
    });
  };

  getCampaignList = (selectedVendorId) => {
    this.props.dispatch(fetchCampaignList(selectedVendorId)).then((response) => {
      if (!response) {
        this.setState({
          alertType: "error",
  ...
  getDeclinedData = (selectedVendorId, selectedCampaignId) => {
    this.props.dispatch(fetchDeclinedData(selectedVendorId, selectedCampaignId)).then((response) => {
      if (!response) {
        this.setState({
          alertType: "error",
          alertMessage: response.error,
      ...
      this.setState({
        isFetching: false,
        declinedStatus: this.props.payees.declinedStatus,
        otherReasons: this.props.payees.otherReasons,
  ...
  checkFilterStatus = () => {
    const { amount, source, declinedReason, name, searchType } = this.state;
    if (
      source.length === 0 &&
      declinedReason.length === 0 &&
      name.length === 0 &&
      searchType === null &&
          ...
          <Paper elevation={0} className={classes.enrollmentGraphs}>
            <Box mx={1}>
              <DeclinedData
                ...
                selectedCurrencyDeclined={selectedCurrencyDeclined}
              />
            </Box>
          </Paper>
        </Box>
                    ...
                    <Grid item xs={3}>
                      <Box mx={1}>
                        ...
                        >
                          ...
                          {declinedReasonList &&
                            declinedReasonList.map(
                              (item, i) => (
                                <MenuItem key={item.bucketId} value={item.declinedReason}>
                                  {item.declinedReason}
                                </MenuItem>
                              )
                            )}
