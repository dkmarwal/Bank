...
import {
    fetchPayeesList, fetchColumnsList,
    fetchOnboardingSrcList, fetchResultsList,
...
class Payees extends Component {
  ...
  getColumnListsName = () => {
    this.props.dispatch(fetchColumnsList("Enrolled")).then((response) => {
      if (!response) {
        this.setState({
          alertType: "error",
          alertMessage: this.props.payees.error,
      ...
      this.setState({
        columns: this?.props?.payees?.columnList.map((item) => ({
          ...item,
          isChecked: item.isCheckEnrolled,
          key: item.infokey,
        })),
      });
    });
  ...
  getExpectedResultList = () => {
    this.props.dispatch(fetchResultsList()).then((response) => {
      if (!response) {
        this.setState({
          alertType: "error",
          alertMessage: this.props.payees.error,
  ...
  cellText = (data, text, key) => {
    ...
    if (typeof text === "object") {
      ...
      // return <Typography variant="subtitle2" style={{color: data.expectedColorCode}}>{text}</Typography>;
      return <LightBlueBtn color={data.expectedColorCode}> {text} </LightBlueBtn> 
    }
     else {
  ...
  checkFilterStatus = () => {
    const { amount, source, expectedResult, name, searchType } = this.state;
    if (
      source.length === 0 &&
                ...
                <Grid container direction="row" spacing={3}>
                  ...
                  >
                    <Box mr={3} cursor="pointer" fontSize={14} color="#9E9E9E">
                      <div>Filters - </div>
                      {isFilterApplied && (
                        <Box onClick={() => this.clearFilters()} style={{"cursor":"pointer", "borderBottom":"1px solid #eee"}}>
                          {" "}
                          Clear All{" "}
                        </Box>
                      )}
                    ...
                    <Grid item xs={3}>
                      <Box mx={1}>
                        ...
                        >
                          ...
                          {expectedResultList &&
                            expectedResultList.filter(i => i.id !== 4).map((item, i) => (
                              <MenuItem
                                key={item.id}
                                value={item.id}
                              >
              ...
              <TableContainer className={classes.container}>
                <Table>
                  ...
                  >
                    ...
                    {payeesList && payeesList.length === 0 && !isFetching && (
                      <Grid
                        container
                        style={{
                          padding: "10px 0",
