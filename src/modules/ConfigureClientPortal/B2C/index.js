...
import {
  getB2CClientPermissions,
  getB2CPermissionOptions,
  b2cUpdateClientPermissions,
  sendB2CActivationCode,
...
const ConfigureClientPortal = ({
  ...
  const fetchClientPermissions = async () => {
    const response = await getB2CClientPermissions(clientId);
  ...
  const fetchPermissionOptions = async () => {
    const data = await getB2CPermissionOptions();
  ...
  function createPermissionGroups(data) {
    const GroupPermissions = (data && data.length > 0 && data.reduce(
  ...
  const onNext = async () => {
    ...
    if (
      ...
      const response = await b2cUpdateClientPermissions(permissionGranted);
      ...
      if (updatePermission) {
        ...
        if (selectedOnboardType !== "self" && !error) {
          ...
          setNextLoading(false);
          history.push("/clientOnboard/b2c/clientProfile");
        } else {
          setNextLoading(false);
  ...
  const PermissionsOptionList = userPermissionOptions && userPermissionOptions.map(
  ...
  const onSendActivationCode = async () => {
    ...
    setOpenModal(!openModal);
    history.push("/clientOnboard/b2c/ClientRegistrationCompleted");
  };
  ...
  const callSendActivationCodeAPI = async (clientId, isVerified) => {
    ...
    setBtn(true);
    if (!btn) {
      const resp = await sendB2CActivationCode(data);
      const { error } = resp;
      if (error) {
        // MessageError();
      }
...
const mapStateToProps = ({ clients = {}, user }) => {
  ...
  return { clientId, isHippa, parentId, user: user };
};
