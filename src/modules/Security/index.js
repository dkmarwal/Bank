...
class Security extends React.Component {
    ...
    saveIP = () => {
        ...
        if (isErr && isActive === 1) {
            ...
            this.setState({
                variant: "error",
                errMsg: "Duplicate IP is not allowed"
            })
        }
        else {
            if (isActive === 1 && ipAddresses.indexOf("") != -1) {
                this.setState({
                    variant: "error",
                    errMsg: "IP address can not be empty"
                })
            }
            else {
                this.setState({
                ...
                <Grid container className={classes.SecurityContainer}>
                    <Grid item xs={5} className={classes.headingSec}>
                        <Typography variant="h1">
                            ...
                            <span>Enable IP Address Restriction</span>
                        </Typography >

                        <Typography className={classes.paraTxt}>
                            If enabled, this will restrict access using source IP addresses. Allow access only to specified IP address, you can add up to 5 IP Address.
                        </Typography>

                        {clientIPAdd
                            ? <Typography className={classes.paraTxt}>
                                Your public IP address is: <strong>{clientIPAdd}</strong>
                            </Typography>
                            : null
                        }
                            ...
                            <>
                                <Grid item xs={6} className={classes.IPBox}>
                                    <IPAddress
                                        ...
                                        required={true}
                                        errMsg="Please enter a valid IP address"
                                    />
                                </Grid>
                            </>
                        ))}
                    ...
                    {ipAddresses.length < 5
                        ? <Grid container className={classes.addIPBtn}>
                            ...
                            >
                                <AddIcon /> ADD ANOTHER IP ADDRESS
                            </Button>
                        </Grid>
                        : null
                    }
