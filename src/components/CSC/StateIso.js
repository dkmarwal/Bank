...
export class StateIso extends React.Component {
    state = {
        states: [],
        isLoading: false,
    }
    ...
    render() {
        ...
        const { selectedCountry, onChange, selectedState, required = false, error, helperText, label, name } = this.props;
        return (
            <span>
                <TextField
                    required={required}
                ...
                >
                    {isLoading ? <CircularProgress /> : selectedCountry && states && states.map(s => <MenuItem key={s["name"]} value={s["stateCode"]}>{s["name"]}</MenuItem>)}
                </TextField>
            </span>
        )
    }
